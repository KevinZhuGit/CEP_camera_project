...
data_folders = { 
                ...
                3:['f1_65ms_0.48_4.1_60rows_ptc_v3_t6Exp000058Mask900_PTC/processed_t6Exp000058Mask900_PTC',
                    '../f1_65ms_0.48_3.6.csv', 'f1', 'subbyte_x200_N_870_led_{:04d}.npz'],
                 }
...
if __name__ == '__main__':
    ...
    if(REARRANGE_IMGS):
        ...
        for l in range(0,nuIntensities):
            ...
            def rearrange_img_mp(index_file):
                save_file = os.path.join(data_folder,'subimg_led_{:04d}_{:04d}.npz'.format(TLEDOn,index_file))
    ...
    if(CALCULATE_INTENSITY):
        for l in range(nuIntensities):
            ...
            def calculate_flux_mp(i):
                ...
                save_file = os.path.join(data_folder, 'flux_log_led_{:04d}_{:04d}.png'.format(TLEDOn, i))
                if(os.path.isfile(save_file)):
                    print("This exists already {:04d}".format(i))
                    return 0
    ...
    if(CHECK_IF_VALID):
        img_fom = np.zeros([nuIntensities,nuSamples])
        for l in range(nuIntensities):
            TLEDOn = TLEDOnList[l]
            print("Checking if valid images for led intensity {}".format(TLEDOn))

            for i in range(nuSamples):
                current_file = os.path.join(data_folder, 'flux_led_{:04d}_{:04d}.npy'.format(TLEDOn, i))
                if(os.path.isfile(current_file)):
                    img_i           = np.load(current_file)        
                    img_fom[l,i]    = np.mean(img_i[17:35,350:450].flatten())
                else:
                    print(i,end=' ')
    ...
    if(CALCULATE_MEAN):
        ...
        for l in range(nuIntensities):
            TLEDOn = TLEDOnList[l]
            print("Calculating mean for led intensity {}".format(TLEDOn))
            ...
            for i in range(nuSamples):
                ...
                img_i = np.load(os.path.join(data_folder, 'flux_led_{:04d}_{:04d}.npy'.format(TLEDOn, i)))
                img_i = np.interp(img_i,(least_count,nuSubs),(1,2**16-1))
                img_sum += img_i
                n       += 1
            img_mean = img_sum / n
            np.save(os.path.join(data_folder, 'mean_led_{:04d}.npy'.format(TLEDOn)),img_mean)
            print("\nDone in {}".format(time.time()-tic));tic=time.time()
    ...
    if(CALCULATE_VARIANCE):
        for l in range(nuIntensities):
            TLEDOn = TLEDOnList[l]
            print("Calculating variance for led intensity {}".format(TLEDOn))
            img_sum = np.zeros([rows,cols])
            img_mean = np.load(os.path.join(data_folder, 'mean_led_{:04d}.npy'.format(TLEDOn)))
            least_count = nuSubs/(2**16-1)
            ...
            for i in range(nuSamples):
                ...
                n       += 1
            img_var = img_sum / n
            np.save(os.path.join(data_folder, 'variance_led_{:04d}.npy'.format(TLEDOn)),img_var)
            print("\nDone in {}".format(time.time()-tic));tic=time.time()


    if(PLOT_MEAN_VS_VARIANCE):
        img_mean_all = np.zeros(nuIntensities)
        img_var_all  = np.zeros(nuIntensities)

        for l in range(nuIntensities):
            TLEDOn = TLEDOnList[l]
    
            img_mean = np.load(os.path.join(data_folder,     'mean_led_{:04d}.npy'.format(TLEDOn)))
            img_var  = np.load(os.path.join(data_folder, 'variance_led_{:04d}.npy'.format(TLEDOn)))

            img_mean_all[l] = img_mean[30,340]
            img_var_all[l]  =  img_var[30,340]
