...
if(1):
	...
	saveFolder 	= "./image/20210516_2000/processed/mask{:01d}_rep{:02d}_tap{:01d}/"
	matFolder 	= "./image/20210516_2000/rawData/matlab/mask{:01d}_rep{:02d}_tap{:01d}/"
	...
	tapList		= [1,2]
	repList 	= [1,2,4,8,32]
	# expList 	= np.concatenate((np.logspace(np.log10(54),np.log10(20000),30),np.arange(25000,100000,5000),np.arange(100000,300000,20000),np.arange(300000,500000,50000)))
	...
	# for mask_num in maskList:
		...
		for tap in tapList:
			for exp in expList:
				...
				fMatSave = os.path.join(matFolder.format(mask_num,rep_num,tap),"{:06d}.mat".format(int(exp*mask_num)))
				...
				if(os.path.isdir(f)):
					...
					k=getImages(f1,nuAvg=nof)
					data = {
						'allImgs':k[0:nof,:,:].astype(np.uint16),
					}

					scipy.io.savemat(fMatSave,data)
		
					fNpySave = os.path.join(saveFolder.format(mask_num,rep_num,tap),"avg_{:06d}.npy".format(int(exp)))
					np.save(fNpySave,k[-2,:,:])
					fNpySave = os.path.join(saveFolder.format(mask_num,rep_num,tap),"std_{:06d}.npy".format(int(exp)))
...
mask,tap = 2,2
readFolder = './image/20201108/processed/mask{}_tap{}/'.format(mask,tap)
